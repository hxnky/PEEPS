					...
					function() {
						...
						$
								.ajax({
									...
									url : "http://52.79.234.178:8080/post/rest/member/post/detail?idx="
											+ postIdx,
									type : 'GET',
									success : function(data) {
										...
										// 2021.03.08 효영 알람 추가
										var almpostHTML = '<input type="hidden" id="almpost2" value="'+data.p_idx+'">';
										almpostHTML += '<input type="hidden" id="almrm2" value="'+data.member_idx+'">';
										$('.likeBtn').append(almpostHTML);
										...
										if (sessionMidx == data.member_idx) {
											...
											var Btn = '<a class="deleteBtn" href="javascript:deletePost('
													+ data.p_idx + ');">삭제</a>';
											Btn += '<a class="editBtn" href="<c:url value="/post/edit?idx='
													+ data.p_idx
													+ '" />">수정</a>';
											$('.deBtn').append(Btn);
										}
						...
						$
								.ajax({
									...
									url : "http://52.79.234.178:8080/post/rest/member/post/detail/image?idx="
											+ postIdx,
									type : 'post',
									success : function(data) {
		...
		if (confirm('삭제하시겠습니까?')) {
			...
			$
					.ajax({
						...
						url : "http://52.79.234.178:8080/post/rest/member/post/delete?idx="
								+ pidx,
						type : 'GET',
						success : function(data) {
							var memberid = "${id}";
							/* window.location.href="http://localhost:8081/post/main/jhS2"; */
							window.location.href = "${pageContext.request.contextPath}/user/mypage?id="
									+ memberid;
...
<script>
	...
	function loadComment() {
		...
		$
				.ajax({
					...
					url : "http://52.79.234.178:8080/post/rest/cmt/select?PostNO="
							+ postIdx,
					type : 'get',
					data : {
						"postIdx" : postIdx
					...
					success : function(data) {
						...
						var almpostHTML = almpostHTML += '<input type="hidden" id="almrm3" value="'+member_idx+'">';
                        $('.cmtdiv').append(almpostHTML);
	...
	$(function() {
		...
		var almpostHTML = almpostHTML += '<input type="hidden" id="almpost3" value="'+postIdx+'">';
        $('.cmtdiv').append(almpostHTML);
