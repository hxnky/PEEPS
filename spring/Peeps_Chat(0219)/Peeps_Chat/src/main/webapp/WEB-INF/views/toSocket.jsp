...
<body>
	...
	<div id="my_modal_wer">
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach items="listNav()" var="sender" varStatus="i">
					<table id="alarm">
						<tr>
							<td><a href="/alarm/select ${sender.m_idx}"> <c:set
										var="type" value="${alarm.type}" /> <c:choose>
										<c:when test="${type eq 'comment'}">
											<img id="profile_modal"
												src="<c:url value="/fileupload/${sender.m_photo}"/>">
										</c:when>
